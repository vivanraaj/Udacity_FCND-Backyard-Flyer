...
class BackyardFlyer(Drone):
	...
	def local_position_callback(self):
		...
		"""
		# find a way to transition takeoff to waypoints.
		if self.flight_phase == States.TAKEOFF:
			# coordinate conversion using z axis
		...
		#if still in waypoint mode
		elif self.flight_phase == States.WAYPOINT:
			# check the Euclidean distance of next waypoint and current waypoint
			if np.linalg.norm(self.target_position[0:2] - self.local_position[0:2]) < 1.0:
				# if there are still waypoints to travel to, do the transition
	...
	def waypoint_transition(self):
		...
		# need to get the first waypoint from the array
		self.target_position = self.all_waypoints.pop(0)
		#1. Command the next waypoint position
		# play with heading value in radians
